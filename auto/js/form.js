...
function has3d(){
  ...
  function load_user_form()
  {
    ...
    for (key in cook)
    {
      ...
      if (cook[key].substring(0,9)=='EartForm' || cook[key].substring(1,10)=='EartForm')
      {
...
function save_user_form(id, real_id)
{
  ...
  document.cookie="EartForm_"+id+"=true&"+data+"; expires="+date.toUTCString();
}
...
function increment_form(idd)
{
  ...
  jQuery.ajax({
    dataType: 'json',
    type: "POST",
    url: EartFormJS.ajaxurl,
    data: 'id='+id[0]+'&action=eartforms_increment2',
    ...
    if (jQuery('.fileupload-input').length)
    {
      jQuery('.fileupload-input').fileupload({
        ...
        done: function (e, data)
        {
          ...
          jQuery('<i data-key="'+data.result.files.new_name+'" class="eartforms-trash" title="Delete"></i>').appendTo(data.fname).click(function()
          {
            jQuery.ajax({
              url: EartFormJS.ajaxurl,
              type: "POST",
              data: 'action=eartforms_delete_file&key='+encodeURIComponent(jQuery(this).attr('data-key')),
              success: function (response) {
                if (response=='Deleted')
                {
                  jQuery(data.fname).text('');
                  jQuery('<span style="color: red; font-weight: bold; margin-left: -20px"><i class="eartforms-ok" style="color: green"></i></span>').appendTo(data.fname);
...
jQuery('.nform .fileupload-input').attr('data-url', EartFormJS.server);
...
function submit_eartforms(id)
{
  ...
  jQuery.ajax({
    ...
    type: "POST",
    url: EartFormJS.ajaxurl,
    data: 'id='+fid+'&action=eartforms_submit&'+jQuery('#'+id).mySerialize(id)+jQuery('#'+id+' :input').not('.no_show').serialize()+'&title='+title+'&emails='+emails,
    success: function (response)
    {
...
/* Submit the Form */
function submit_eartforms_new(form_id, form_uniq, id)
{
  ...
  jQuery.ajax({
    ...
    type: "POST",
    url: EartFormJS.ajaxurl,
    data: 'id='+fid+'&action=eartforms_submit&'+jQuery('#'+id).mySerialize(form_id)+jQuery('#'+form_id+' :input').not('.no_show').serialize()+'&title='+title+'&emails='+emails+'&multi='+multi,
    success: function (response)
    {
...
function update_date()
{
  ...
  jQuery('.datepickermdy').each(function(){
    ...
    {
      jQuery.get(EartFormJS.locale+"bootstrap-datepicker."+lang+".js", function(){
       jQuery('#'+id2).datepicker(
       {
        'format': dmy,
        'language': lang,
...
jQuery(document).ready(function () {
  ...
  jQuery('body').on('submit','.nform',function(event)
  {
    ...
    submit_eartforms_new(jQuery(this).attr('id'), id[0],id[1]);
    return false;
  });
  ...
  jQuery('.nform').on('click','.label_div',function(){
    ...
    // Pre-Load Image
    var img=new Image();
    img.src=EartFormJS.other+'/img/loader_4.gif';
